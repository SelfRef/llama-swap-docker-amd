...
| llama.cpp master + open [qwen4exp PRs](#qwen4exp) (Qwen3.8-Flash-Next: fixes, perf, NextN/MTP) | `llama-server-qwen4exp` | `llama-server-qwen4exp-rocm` |
...
| `WITH_QWEN4EXP` | `true` | Build `llama-server-qwen4exp` (Vulkan; also its `-rocm` HIP twin when `WITH_ROCM=true`): llama.cpp `QWEN4EXP_COMMIT` + the open qwen4exp PRs merged (see below). `false` skips the stages. |
| `QWEN4EXP_COMMIT` | `master` | llama.cpp revision the qwen4exp PRs are merged onto |
| `QWEN4EXP_PATCHES` | `28121 27941 28104 27992 28136 28118 27952` | Open upstream PRs merged into the qwen4exp builds. Closed PRs are skipped with a notice; a conflicting merge fails the build. `patches/*.patch` are applied on top (unsloth MTP sidecar tensor names, a #28136 merge fix) and also fail the build when they drift. |
...
`llama-server-qwen4exp` (all tags) and `llama-server-qwen4exp-rocm` (`:rocm`) are extra llama.cpp builds for **Qwen3.8-Flash-Next** (arch `qwen4exp`): current master plus the open upstream PRs that the architecture is still waiting on — correctness fixes (#28121, #27941), decode/prefill perf (#27992 O(log n) n-gram index, #28136 direct PLE reads, #28118 on-device speculative checkpoints), NextN/MTP speculative decoding (#28104) and the int8 coopmat matmul (#27952). Local patches in `patches/` make unsloth's `MTP/mtp-*.gguf` sidecar draft heads load (their hyper-connection mixer sits under `blk.N.nextn.hc_head_*`; use the non-`shared` file — the `shared` variant lacks `token_embd`). Typical flags: `--spec-type draft-mtp --spec-draft-hf unsloth/Qwen3.8-Flash-Next-GGUF --spec-draft-model MTP/mtp-Qwen3.8-Flash-Next-Q4_K_M.gguf --spec-draft-n-max 3 -ngld 0 --lazy-mode on-direct` (measured on an RX 7900 XTX: code +64%, 97k-context prose +30%, short prose −8%; keep the draft on the CPU — `-ngld 999` gains nothing and can OOM at long context since `--fit` doesn't budget the draft). The stock `llama-server` stays untouched; drop PRs from `QWEN4EXP_PATCHES` as they merge (the build says so) and retire the binary once the set is empty.
