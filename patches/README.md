# Local patches for the `-next` llama.cpp build

Every `*.patch` here is applied (`git apply`) on top of `NEXT_COMMIT` + the merged
`NEXT_PATCHES` PRs, in glob order. A patch that reverse-applies is treated as
...
both features and merges cleanly.
